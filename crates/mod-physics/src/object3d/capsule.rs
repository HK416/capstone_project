...
impl OrientedCapsule {
    /// direction을 단위벡터로 만들어 Capsule을 생성한다.  
    /// 영벡터가 주어지면 Error를 반환한다.  
    pub fn new(center: glam::Vec3, direction: glam::Vec3, height: f32, radius: f32) -> Result<Self, &'static str> {
    ...
    /// - height는 2 * amount만큼 증가한다.
    /// - radius는 amount만큼 증가한다.
    pub fn inflated(&self, amount: f32) -> OrientedCapsule {
        OrientedCapsule {
            center: self.center - self.direction * amount,
    ...
    /// 캡슐의 아래 구의 중심과 윗 구의 중심을 이은 선분
    pub fn get_seg(&self) -> Segment {
        let direction = glam::Vec3A::from(self.direction);
        let center = glam::Vec3A::from(self.center);
