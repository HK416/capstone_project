...
/// 뼈의 오프셋 데이터입니다.
#[derive(Debug, Clone)]
pub struct BoneOffsets(pub Vec<gmm::Float4x4>);

impl BoneOffsets {
    /// 뼈 오프셋 데이터가 비어있는 경우 `true`를 반환합니다.
    #[inline]
    pub fn is_empty(&self) -> bool {
        self.count() == 0
    }

    /// 뼈 오프셋 데이터의 바이트 단위 크기를 반환합니다.
    #[inline]
    pub fn size(&self) -> usize {
        self.stride() * self.count()
    }

    /// 뼈 오프셋 데이터의 갯수를 반환합니다.
    #[inline]
    pub fn count(&self) -> usize {
        self.0.len()
    }
    ...
    #[inline]
    pub fn stride(&self) -> usize {
        size_of::<gmm::Float4x4>()
    }

    /// 뼈 오프셋 데이터의 바이트 배열을 반환합니다.
    #[inline]
    pub fn as_bytes(&self) -> &[u8] {
        cast_slice(&self.0)
    }
