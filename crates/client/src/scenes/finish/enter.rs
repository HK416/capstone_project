...
use crate::{
    ...
    component::{
        ...
        BrightRenderTarget, BulletRenderPipelineTransparency, CameraDataLayout, CameraResource,
        ...
        GaussianBlurPipeline, GlobalLight, GlobalLightDataLayout, HaloRenderPipeline,
        LightSetResource, LightTransformDataLayout, MaterialKind, Mesh, MeshRenderer, OpaqueMap,
        Parent, Projection, RevealRenderTarget, ShadowMap, Sibling, SkinnedMeshRenderer,
        SkinningAnimation, Skybox, SkyboxDataLayout, SkyboxRenderPipeline, StageBakePipeline,
        StageRenderPipeline, ThirdPersonCamera, ToParentTrans, TransparentMap, TreeRenderPipeline,
        WorldTransform, SHADOW_FORMAT,
    },
    ...
    fn on_draw(
        ...
        encoder.push_debug_group("opaque pass");
        {
            ...
            self.draw_damage_particle(
                DamageFontRenderPipeline::get_or_init(&device, SWAPCHAIN_FORMAT, DEPTH_FORMAT),
                &camera_resource,
                &mut rpass,
            );
        ...
        encoder.push_debug_group("transparent pass");
        {
            ...
            for ((mesh, kind), material_resources) in self.transparent_map.iter() {
                match kind {
                    MaterialKind::Bullet => {
                        draw_bullet(
                            &mesh,
                            BulletRenderPipelineTransparency::get_or_init(&device, DEPTH_FORMAT),
                            &camera_resource,
                            light_set_resource,
                            material_resources,
                            &mut rpass,
                    ...
                    MaterialKind::EnergyBullet => {
                        draw_energy_bullet(
                            &mesh,
                            EnergyBulletRenderPipeline::get_or_init(&device, DEPTH_FORMAT),
                            &camera_resource,
                            light_set_resource,
                            material_resources,
                            &mut rpass,
