...
use crate::component::{
    AccumRenderTarget, AttributeKind, BrightRenderTarget, CameraResource, FxShieldInstance,
    FxShieldInstanceDataLayout, FxShieldResource, Mesh, ParticleResource, RevealRenderTarget,
};

/// 방어막 파티클을 그리는 렌더링 파이프라인입니다.
pub struct FxShieldRenderPipeline;

static PIPELINE: OnceLock<wgpu::RenderPipeline> = OnceLock::new();

impl FxShieldRenderPipeline {
    /// [wgpu::ShaderModule]을 반환합니다.
    fn create_shader_module(device: &wgpu::Device) -> wgpu::ShaderModule {
        unsafe {
            let desc = wgpu::include_wgsl!(concat!(
                env!("CARGO_WORKSPACE_DIR"),
                "assets/shaders/fx_shield.wgsl"
            ));
    ...
    fn get_pipeline_layout(device: &wgpu::Device) -> wgpu::PipelineLayout {
        device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("PipelineLayout(Fx(Shield))"),
            bind_group_layouts: &[
                CameraResource::bind_group_layout(device),
                FxShieldResource::bind_group_layout(device),
            ],
            ...
            device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                vertex: wgpu::VertexState {
                    ...
                    buffers: &[
                        ...
                        // 2번 입력 속성: 인스턴스 데이터
                        wgpu::VertexBufferLayout {
                            array_stride: core::mem::size_of::<FxShieldInstanceDataLayout>()
                                as wgpu::BufferAddress,
                            attributes: &[
                                wgpu::VertexAttribute {
                                    offset: offset_of!(FxShieldInstanceDataLayout, x_axis)
                                        as wgpu::BufferAddress,
                                    shader_location: 2,
                                    format: wgpu::VertexFormat::Float32x4,
                                },
                                wgpu::VertexAttribute {
                                    offset: offset_of!(FxShieldInstanceDataLayout, y_axis)
                                        as wgpu::BufferAddress,
                                    shader_location: 3,
                                    format: wgpu::VertexFormat::Float32x4,
                                },
                                wgpu::VertexAttribute {
                                    offset: offset_of!(FxShieldInstanceDataLayout, z_axis)
                                        as wgpu::BufferAddress,
                                    shader_location: 4,
                                    format: wgpu::VertexFormat::Float32x4,
                                },
                                wgpu::VertexAttribute {
                                    offset: offset_of!(FxShieldInstanceDataLayout, w_axis)
                                        as wgpu::BufferAddress,
                                    shader_location: 5,
                                    format: wgpu::VertexFormat::Float32x4,
                                },
...
pub fn draw_fx_shield_effect<'a>(
    ...
    rpass.set_pipeline(FxShieldRenderPipeline::get_or_init(device, DEPTH_FORMAT));
    ...
    for indices in mesh.submeshes() {
        ...
        rpass.draw_indexed(0..indices.count(), 0, 0..instance_buffer.num_instance());
    }
}
