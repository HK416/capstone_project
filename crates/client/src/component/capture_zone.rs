//! 점령 지역을 그리는 그래픽스, 컴퓨트 파이프라인 코드를 관리합니다.
//!

use std::sync::{Arc, OnceLock};

use crate::component::{CameraResource, CaptureZoneMaterialResource, MeshResource};

use super::WeightedBlendedOITResource;

/// 점령 지역을 그리는 렌더링 파이프라인입니다.
pub struct CaptureZoneRenderPipeline;

/// 점령 지역을 그리는 그래픽스 파이프라인 인스턴스입니다.
static PIPELINE: OnceLock<Arc<wgpu::RenderPipeline>> = OnceLock::new();

impl CaptureZoneRenderPipeline {
    /// [wgpu::ShaderModule]을 반환합니다.
    fn create_shader_module(device: &wgpu::Device) -> wgpu::ShaderModule {
        unsafe {
            let desc = wgpu::include_wgsl!(concat!(
                env!("CARGO_WORKSPACE_DIR"),
                "/assets/shaders/capture_zone.wgsl"
            ));
    ...
    fn get_pipeline_layout(device: &wgpu::Device) -> wgpu::PipelineLayout {
        device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[
                ...
                CaptureZoneMaterialResource::bind_group_layout(device),
            ],
            ...
            .get_or_init(|| {
                ...
                Arc::new(
                    device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                        label: Some("RenderPipeline(CaptureZone)"),
                        layout: Some(&layout),
