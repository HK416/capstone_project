...
pub struct BulletMaterialUniform(Arc<wgpu::Buffer>);

impl BulletMaterialUniform {
    /// 유니폼 버퍼의 크기입니다.
    pub const SIZE: wgpu::BufferAddress =
        core::mem::size_of::<BulletMaterialDataLayout>() as wgpu::BufferAddress;
    ...
    /// 새로운 유니폼 버퍼를 생성합니다.
    pub fn new(label: Option<&str>, device: &wgpu::Device, data: BulletMaterialDataLayout) -> Self {
                ...
                Ok(_) => {
                    {
                        ...
                        let layout: &mut BulletMaterialDataLayout =
...
static_assertions::const_assert_ne!(BulletMaterialUniform::SIZE, 0);
static_assertions::const_assert_eq!(
    BulletMaterialUniform::SIZE as usize,
    ...
    pub fn bind_group_layout(device: &wgpu::Device) -> &'static wgpu::BindGroupLayout {
        ...
        LAYOUT.get_or_init(|| {
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("BindGroupLayout(BulletMaterialResource)"),
                entries: &[
                    // 0번 바인딩: 총알 재질 데이터 유니폼
                    BulletMaterialUniform::bind_group_layout_entry(wgpu::ShaderStages::FRAGMENT, 0),
                ],
            })
        })
    }
