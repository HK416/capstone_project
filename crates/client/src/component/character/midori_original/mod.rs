//! `Midori_Original` 모델과 관련된 코드를 관리합니다.
//!
...
lazy_static! {
    pub static ref CHARACTER_ATTRIBUTE: CharacterAttributes = {
        let json = include_str!(concat!(
            ...
            "/assets/characters/midori_original/attribute.json"
        ));
...
pub fn spawn_fx_muzzle_effect(
    ...
    let transform = ToParentTrans(glam::Mat4::from_scale(glam::vec3(0.4, 0.4, 0.4)));
...
pub fn spawn_skill_fx_muzzle_effect(
    ...
    let transform = ToParentTrans(glam::Mat4::from_scale(glam::vec3(0.5, 0.5, 0.5)));
    let mut builder_0 = EntityBuilder::new();
    builder_0.add_bundle((
        mesh.clone(),
        parent,
