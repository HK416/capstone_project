//! 캐릭터 모델의 `MoveToEnd` 애니메이션과 관련된 코드를 관리합니다.
//!
...
///
pub fn animate_character_when_move_to_end<Tag: Copy + Component>(
    ...
    let motion = motions
        .get(MOVE_TO_END_ANIMATION)
        .expect("no such animation data!");

    // 애니메이션 키 프레임을 샘플링합니다.
    let time_point_0 = movement_state_timer
        .0
        .min(character_attributes.move_end_normal_duration);
