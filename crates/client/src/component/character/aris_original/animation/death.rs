//! 캐릭터 모델의 `Death` 애니메이션과 관련된 코드를 관리합니다.
//!
...
///
pub fn animate_character_when_death<Tag: Copy + Component>(
    ...
    let motion = motions
        .get(VITAL_DEATH_ANIMATION)
        .expect("no such animation data!");

    // 애니메이션 키 프레임을 샘플링합니다.
    let time_point_0 = action_state_timer
        .0
        .min(character_attribute.vital_death_duration);
