//! 캐릭터 모델의 `VictoryEnd` 애니메이션과 관련된 코드를 관리합니다.
//!
...
///
pub fn animate_character_when_victory_end<Tag: Copy + Component>(
    ...
    // `*_Victory_End` 애니메이션을 가져옵니다.
    let motion = motions
        .get(VICTORY_END_ANIMATION)
        .expect("no such animation data!");

    // 애니메이션 키 프레임을 샘플링합니다.
    let time_point_0 = action_state_timer.get() % character_attribute.victory_end_duration;
