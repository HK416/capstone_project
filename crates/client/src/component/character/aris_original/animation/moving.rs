//! 캐릭터 모델의 `Moving` 애니메이션과 관련된 코드를 관리합니다.
//!
...
///
pub fn animate_character_when_moving<Tag: Copy + Component>(
    ...
    let motion = motions
        .get(MOVING_ANIMATION)
        .expect("no such animation data!");

    // 애니메이션 키 프레임을 샘플링합니다.
    let time_point_0 = movement_state_timer.get() % character_attribute.move_ing_duration;
