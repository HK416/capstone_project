...
use crate::component::{
    BrightRenderTarget, CameraResource, EyeMouthMaterialResource, LightSetResource, ShadowResource,
    SkinnedMeshResource,
};

/// 캐릭터의 눈과 입을 그리는 렌더링 파이프라인입니다.
pub struct EyeMouthRenderPipeline;

/// 캐릭터의 눈과 입을 그리는 그래픽스 파이프라인 인스턴스입니다.
static PIPELINE: OnceLock<wgpu::RenderPipeline> = OnceLock::new();

impl EyeMouthRenderPipeline {
...
/// 캐릭터 눈과 입의 그림자를 생성하는 파이프라인입니다.
pub struct EyeMouthBakePipeline;

/// 캐릭터 눈과 입의 그림자를 생성하는 그래픽스 파이프라인 인스턴스입니다.
static BAKE_PIPELINE: OnceLock<wgpu::RenderPipeline> = OnceLock::new();

impl EyeMouthBakePipeline {
    /// [wgpu::ShaderModule]을 반환합니다.
    fn create_shader_module(device: &wgpu::Device) -> wgpu::ShaderModule {
        unsafe {
            let desc = wgpu::include_wgsl!(concat!(
                env!("CARGO_WORKSPACE_DIR"),
                "/assets/shaders/character_eye_mouth_bake.wgsl"
            ));
    ...
    fn get_pipeline_layout(device: &wgpu::Device) -> wgpu::PipelineLayout {
        device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("PipelineLayout(Bake(EyeMouth))"),
            bind_group_layouts: &[
    ...
    pub fn get_or_init(
        ...
        BAKE_PIPELINE.get_or_init(|| {
            let module = Self::create_shader_module(device);
            let layout = Self::get_pipeline_layout(device);
            device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                label: Some("RenderPipeline(Bake(EyeMouth))"),
                layout: Some(&layout),
                vertex: wgpu::VertexState {
                    module: &module,
