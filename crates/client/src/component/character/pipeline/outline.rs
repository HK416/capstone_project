#![allow(dead_code)]
//! 캐릭터의 헤일로 외곽선을 그리는 그래픽스, 컴퓨트 파이프라인 코드를 관리합니다.
//!

use std::sync::OnceLock;

use crate::component::{
    BrightRenderTarget, CameraResource, HaloOutlineMaterialResource, MeshResource,
};

/// 캐릭터의 헤일로 외곽선을 그리는 렌더링 파이프라인입니다.
pub struct HaloOutlineRenderPipeline;

/// 캐릭터의 헤일로 외곽선을 그리는 그래픽스 파이프라인 인스턴스입니다.
static PIPELINE: OnceLock<wgpu::RenderPipeline> = OnceLock::new();

impl HaloOutlineRenderPipeline {
    /// [wgpu::ShaderModule]을 반환합니다.
    fn create_shader_module(device: &wgpu::Device) -> wgpu::ShaderModule {
        unsafe {
            let desc = wgpu::include_wgsl!(concat!(
                env!("CARGO_WORKSPACE_DIR"),
                "/assets/shaders/character_halo_outline.wgsl"
            ));
    ...
    fn get_pipeline_layout(device: &wgpu::Device) -> wgpu::PipelineLayout {
        device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("PipelineLayout(HaloOutline)"),
            bind_group_layouts: &[
                CameraResource::bind_group_layout(device),
                MeshResource::bind_group_layout(device),
                HaloOutlineMaterialResource::bind_group_layout(device),
            ],
    ...
    pub fn get_or_init(
        ...
        PIPELINE.get_or_init(|| {
            ...
            device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                label: Some("RenderPipeline(HaloOutline)"),
                layout: Some(&layout),
