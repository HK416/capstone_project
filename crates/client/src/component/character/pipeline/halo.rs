...
use crate::component::{BrightRenderTarget, CameraResource, HaloMaterialResource, MeshResource};

/// 캐릭터의 헤일로를 그리는 렌더링 파이프라인입니다.
pub struct HaloRenderPipeline;

/// 캐릭터의 헤일로를 그리는 그래픽스 파이프라인 인스턴스입니다.
static PIPELINE: OnceLock<wgpu::RenderPipeline> = OnceLock::new();

impl HaloRenderPipeline {
    ...
    fn get_pipeline_layout(device: &wgpu::Device) -> wgpu::PipelineLayout {
        device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[
                ...
                HaloMaterialResource::bind_group_layout(device),
            ],
