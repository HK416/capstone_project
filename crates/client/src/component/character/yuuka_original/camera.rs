//! `Momoi_Original` 모델의 카메라 설정과 관련된 코드를 관리합니다.
//!
...
fn attack_wave_camera_effect(
    ...
    *camera_rel_pos = rel_pos;
    if (TIME_POINT_0..TIME_POINT_1).contains(&action_state_timer.0) {
        ...
        *camera_fov_y = fov_y + WAVE_OFFSET * s;
    } else if (TIME_POINT_2..TIME_POINT_3).contains(&action_state_timer.0) {
        let t = (action_state_timer.0 - TIME_POINT_2) as f32 / WAVE_DURATION_0 as f32;
        let s = t * t / (t * t + (1.0 - t) * (1.0 - t));
        *camera_fov_y = fov_y + WAVE_OFFSET * s;
