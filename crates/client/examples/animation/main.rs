...
fn main() {
    ...
    AppBuilder::new(Box::new(ExampleScene::default()))
        .with_window_title("Example: Animation")
...
impl ExampleScene {
    ...
    /// 모델 에셋을 생성합니다.
    fn spawn_aris_original_model(&mut self, app: &dyn AppHandle) {
...
impl GameScene for ExampleScene {
    ...
    fn on_enter(
        ...
        self.spawn_aris_original_model(app);
        Ok(())
    }
    ...
    fn on_update(
        ...
        if let Some(animation) = self.animations.get(self.anim_index) {
            ...
            for bone in keyframe.bones() {
                ...
                update_hierarchy(&mut self.world, None, root_entity);

                let iter = bone.target().bones().iter()
                    .map(|&entity| **self.world.get::<&WorldTransform>(entity).unwrap())
        ...
        let (
            ...
            world_transform, 
            object
        ) = self.world.query_one_mut::<QueryPlayer>(self.player).unwrap();
        (**transform) = (**transform) * rotation;
        (**world_transform) = **transform;

        // // 계층 구조를 갱신합니다.
        update_hierarchy(&mut self.world, None, self.player);
