...
fn main() {
    ...
    let root = match result {
        Ok(root) => root, 
        Err(e) => panic!("Failed to parse asset list file for the following reason: {}", e)
    };
    ...
    if !src_path.is_dir() {
        panic!("Asset directory does not exist! (PATH:{})", src_path.display());
    }
    ...
    if build_path.is_dir() {
        ...
        if let Err(e) = fs_extra::dir::create(&dst_path, true) {
            panic!("Failed to create build target assets directory for the following reason: {}", e)
        }
...
fn copy_asset(src: PathBuf, dst: PathBuf, hierarchy: &Hierarchy, options: &CopyOptions) {
    ...
    for (dir, node) in hierarchy.directories.iter() {
        ...
        if let Err(e) = fs_extra::dir::create(&dst, true) {
            panic!("Failed to create asset directory for the following reason: {}", e)
        }
