...
impl GridMap2D {
    ...
    pub fn from_stage(
        ...
        log::info!("[GRID MAP] Building 2D grid map: size={:.1}m, grid={:.1}m", map_size, grid_size);
        ...
        log::info!("[GRID MAP] Grid dimensions: {}x{} = {} cells", width, height, width * height);
        ...
        let safe_character_radius = if character_radius.is_finite() && character_radius > 0.0 {
            ...
            log::warn!("[GRID MAP] Invalid character_radius: {}, using default 0.5m", character_radius);
            0.5 // 기본값
        };
        ...
        log::info!("[GRID MAP] Grid-based safety margin: 2 grids = {:.2}m (character radius {:.2}m ignored in grid system)",
                  safety_margin, safe_character_radius);
                        ...
                        if collision_result {
                            ...
                            if detailed_collision_count < 10 {
                                let collider_type = match collider {
                                    ...
                                    mod_physics::collision::Collider::OrientedCapsule(_) => "OrientedCapsule",
                                    mod_physics::collision::Collider::Sphere(_) => "Sphere",
                                };
                                log::debug!("[GRID MAP] Obstacle detected at grid({},{}) world({:.1},{:.1}) - type: {}", 
                                           grid_x, grid_y, world_x, world_z, collider_type);
        ...
        let passable_percentage = ((width * height - obstacle_count) as f32 / (width * height) as f32) * 100.0;
        log::info!("[GRID MAP] Map analysis: {}/{} passable ({:.1}%), {} obstacles", 
                  width * height - obstacle_count, width * height, passable_percentage, obstacle_count);
        
        log::info!("[GRID MAP] Collision detection: {} detailed collisions logged (safety margin: {:.2}m)", 
                  detailed_collision_count, safety_margin);
        ...
        log::info!("[GRID MAP] Stage has {} total colliders for obstacle detection", total_colliders);
    ...
    pub fn from_csv(csv_path: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        log::info!("[GRID MAP] Loaded CSV grid map: {}x{} cells, grid_size={:.1}m, bounds=({:.1},{:.1},{:.1},{:.1})", 
                  width, height, grid_size, min_x, min_z, max_x, max_z);
    ...
    pub fn world_to_grid(&self, world_pos: Vec3A) -> Option<(usize, usize)> {
        ...
        if !world_pos.is_finite() {
            log::warn!("[GRID MAP] Invalid world_pos in world_to_grid: {:?}", world_pos);
            return None;
        }
        ...
        if grid_x >= 0 && grid_x < self.dimensions.0 as i32 && 
           grid_z >= 0 && grid_z < self.dimensions.1 as i32 {
            ...
            unsafe {
                if !LOGGED_BOUNDS_ERROR {
                    log::debug!("[GRID MAP] Out of bounds: pos={:?}, signed_grid=({},{}), array_grid=({},{}), dims={:?}", 
                              world_pos, grid_x_signed, grid_z_signed, grid_x, grid_z, self.dimensions);
    ...
    pub fn is_walkable(&self, world_pos: Vec3A) -> bool {
        // 입력값 안전성 검사
        if !world_pos.is_finite() {
            log::warn!("[GRID MAP] Invalid world_pos in is_walkable: {:?}", world_pos);
            return false;
        }
        ...
        if world_pos.x < -half_width || world_pos.x > half_width ||
           world_pos.z < -half_height || world_pos.z > half_height {
            // 범위 밖이면 false (안전하게 차단)
            static mut LOGGED_BOUNDS_WARNING: bool = false;
            unsafe {
                if !LOGGED_BOUNDS_WARNING {
                    log::debug!("[GRID MAP] Position outside map bounds: {:?}, bounds=({:.1},{:.1}) to ({:.1},{:.1})", 
                              world_pos, -half_width, -half_height, half_width, half_height);
        ...
        if let Some((grid_x, grid_z)) = self.world_to_grid(world_pos) {
            ...
            if grid_x >= self.dimensions.0 || grid_z >= self.dimensions.1 {
                log::warn!("[GRID MAP] Grid coordinates out of range: ({},{}) >= ({},{})", 
                          grid_x, grid_z, self.dimensions.0, self.dimensions.1);
                return false;
            }
            ...
            if index < self.obstacle_grid.len() {
                ...
                unsafe {
                    if !LOGGED_GRID_CHECK {
                        log::debug!("[GRID MAP] Basic walkability check: pos {:?} -> grid({},{}) index={} -> walkable={}", 
                                  world_pos, grid_x, grid_z, index, is_walkable);
                        LOGGED_GRID_CHECK = true;
                    }
                }
                is_walkable
            } else {
                log::warn!("[GRID MAP] Index {} out of bounds for grid size {}", index, self.obstacle_grid.len());
                false
            }
        } else {
            // 맵 경계 밖은 통행 불가
            ...
            if index < self.obstacle_grid.len() {
                ...
                if is_walkable {
                    ...
                    for (dx, dz) in directions.iter() {
                        ...
                        if check_x >= 0 && check_x < self.dimensions.0 as i32 &&
                           check_z >= 0 && check_z < self.dimensions.1 as i32 {
                            let check_index = (check_z as usize) * self.dimensions.0 + (check_x as usize);
                            if check_index < self.obstacle_grid.len() && self.obstacle_grid[check_index] {
                    ...
                    if adjacent_obstacles >= 3 {
                        ...
                        unsafe {
                            if !LOGGED_CORNER_BLOCK {
                                log::debug!("[GRID SAFETY] Blocked corner/narrow passage at ({},{}) - {} adjacent obstacles", 
                                          grid_x, grid_z, adjacent_obstacles);
    ...
    pub fn print_stats(&self) {
        ...
        let obstacle_cells = self.obstacle_grid.iter().filter(|&&blocked| blocked).count();
        let passable_cells = total_cells - obstacle_cells;
        
        log::info!("[GRID MAP STATS] Total: {}, Passable: {}, Obstacles: {}, Passable: {:.1}%",
                  total_cells, passable_cells, obstacle_cells, 
    ...
    pub fn world_to_grid_with_bounds(&self, world_pos: Vec3A) -> Option<(usize, usize)> {
        ...
        if array_x >= 0 && array_x < self.dimensions.0 as i32 && 
           array_z >= 0 && array_z < self.dimensions.1 as i32 {
...
impl GridPos {
    ...
    pub fn from_world_pos_with_bounds(world_pos: Vec3A, grid_size: f32, bounds: (f32, f32, f32, f32)) -> Self {
        let safe_grid_size = if grid_size > 0.0 && grid_size.is_finite() {
            grid_size
        } else {
            log::error!("[GRID POS] Invalid grid_size: {}, using fallback 1.0", grid_size);
            1.0
        };
        
        let safe_x = if world_pos.x.is_finite() { world_pos.x } else { 0.0 };
        let safe_z = if world_pos.z.is_finite() { world_pos.z } else { 0.0 };
        ...
        Self { x: grid_x, z: grid_z }
    }
    
    /// GridMap2D의 bounds와 호환되는 월드 좌표 변환
    pub fn to_world_pos_with_bounds(&self, grid_size: f32, base_height: f32, bounds: (f32, f32, f32, f32)) -> Vec3A {
        let safe_grid_size = if grid_size > 0.0 && grid_size.is_finite() {
            grid_size
        } else {
            1.0
        ...
        let safe_height = if base_height.is_finite() { base_height } else { 1.0 };
    ...
    pub fn to_world_pos(&self, grid_size: f32, base_height: f32) -> Vec3A {
        ...
        let safe_grid_size = if grid_size > 0.0 && grid_size.is_finite() {
            ...
            log::error!("[GRID POS] Invalid grid_size in to_world_pos: {}, using fallback 1.0", grid_size);
            1.0
        };
    ...
    pub fn neighbors(&self) -> Vec<(GridPos, u32)> {
        ...
        directions
            ...
            .map(|(dx, dz, cost)| {
                (
                    GridPos::new(self.x + dx, self.z + dz),
                    *cost,
                )
            })
            .collect()
...
pub fn grid_based_astar_pathfind(
    ...
    if distance > 50.0 {
        ...
        log::debug!("[GRID A*] Using hierarchical pathfinding for long distance: {:.1}m", distance);
...
pub fn grid_based_astar_pathfind_standard(
    ...
    log::debug!("[GRID A*] Flexible pathfinding from {:?} to {:?}", start, goal);
    ...
    log::debug!("[GRID A*] Grid coordinates: start_grid={:?}, goal_grid={:?}", start_grid, goal_grid);
    log::debug!("[GRID A*] World coordinates: start={:?}, goal={:?}", start, goal);
    ...
    log::debug!("[GRID A*] Coordinate verification: start {} -> grid {:?} -> {}", 
               start, start_grid, start_converted_back);
    log::debug!("[GRID A*] Coordinate verification: goal {} -> grid {:?} -> {}", 
               goal, goal_grid, goal_converted_back);
    ...
    log::debug!("[GRID A*] Direction verification: direct={:?}, grid={:?}, similarity={:.3}", 
               direct_direction, grid_direction, direction_similarity);
    
    if direction_similarity < 0.7 {
        log::warn!("[GRID A*] WARNING: Grid direction differs significantly from direct direction!");
    ...
    if start_grid.x < -half_width || start_grid.x >= half_width || 
       start_grid.z < -half_height || start_grid.z >= half_height {
        log::warn!("[GRID A*] Start position out of grid bounds: {:?}, grid range: ({},{}) to ({},{})", 
                  start_grid, -half_width, -half_height, half_width-1, half_height-1);
        return None;
    }
    
    if goal_grid.x < -half_width || goal_grid.x >= half_width || 
       goal_grid.z < -half_height || goal_grid.z >= half_height {
        log::warn!("[GRID A*] Goal position out of grid bounds: {:?}, grid range: ({},{}) to ({},{})", 
                  goal_grid, -half_width, -half_height, half_width-1, half_height-1);
        return None;
    }
        ...
        |pos| {
            ...
            pos.extended_neighbors()
                ...
                .filter_map(|(neighbor_pos, base_cost)| {
                    ...
                    // 중앙 원점 기준 그리드 범위 검증
                    if neighbor_pos.x < -half_width || neighbor_pos.x >= half_width || 
                       neighbor_pos.z < -half_height || neighbor_pos.z >= half_height {
                        return None;
                    }
                    ...
                    if grid_map.is_safe_walkable(world_pos) {
                        ...
                        let center_cost = neighbor_pos.center_bias_heuristic(&GridPos::new(0, 0), center_attraction);
                        
                        // 기본 비용과 중앙 지향 비용을 조합
                        let adjusted_cost = ((base_cost as f32 * 0.9) + (center_cost as f32 * 0.1)).max(1.0) as u32;
                        ...
                        unsafe {
                            if DEBUG_COUNT < 5 {
                                log::debug!("[GRID A*] Blocked neighbor: grid={:?}, world={:?}", neighbor_pos, world_pos);
                                DEBUG_COUNT += 1;
                            }
                        }
                        None
    ...
    match result {
        Some((path, total_cost)) => {
            log::info!("[GRID A*] SUCCESS! Flexible path with {} nodes, cost: {}", 
                      path.len(), total_cost);
            ...
            if world_path.len() >= 2 {
                ...
                log::debug!("  Direction alignment: {:.3} (1.0=perfect, -1.0=opposite)", direction_dot);
                
                if direction_dot < -0.1 {
                    log::warn!("[GRID A*] WARNING: First move goes AWAY from goal! (alignment: {:.3})", direction_dot);
                    ...
                    log::debug!("[GRID A*] First move direction looks correct (alignment: {:.3})", direction_dot);
                }
            }
            ...
            if world_path.len() >= 2 {
                ...
                log::info!("[GRID A*] Path analysis: length={:.1}m, direct={:.1}m, detour_ratio={:.2}x", 
                          path_length, direct_distance, detour_ratio);
        ...
        None => {
                    ...
                    pos.extended_neighbors()
                        ...
                        .filter_map(|(neighbor_pos, base_cost)| {
                            ...
                            if grid_map.is_safe_walkable(world_pos) {
                                ...
                                let center_cost = neighbor_pos.center_bias_heuristic(&GridPos::new(0, 0), center_attraction);
                                let adjusted_cost = ((base_cost as f32 * 0.5) + (center_cost as f32 * 0.2)).max(1.0) as u32;
            ...
            match fallback_result {
                Some((path, total_cost)) => {
                    log::info!("[GRID A*] FALLBACK SUCCESS! Ultra-flexible path with {} nodes, cost: {}", 
                              path.len(), total_cost);
                ...
                None => {
                    log::error!("[GRID A*] COMPLETE FAILURE - no path found even with ultra-flexible settings");
                    None
                }
            }
        }
...
where
    F: FnMut(Vec3A) -> bool,
{
    log::debug!("[2D A*] Starting 2D pathfinding from {:?} to {:?} (grid: {:.1}m)", 
    ...
    log::debug!("[2D A*] Grid coordinates: start={:?}, goal={:?}", start_grid, goal_grid);
    ...
    match result {
        Some((path, total_cost)) => {
            log::info!("[2D A*] SUCCESS! Found 2D path with {} nodes, total cost: {}", 
                      path.len(), total_cost);
            ...
            let world_path: Vec<Vec3A> = path
                .into_iter()
                .map(|grid_pos| {
                    grid_pos.to_world_pos(grid_size, base_height)
                })
                .collect();
            ...
            if world_path.len() >= 2 {
                ...
                let direct_distance = ((goal.x - start.x).powi(2) + (goal.z - start.z).powi(2)).sqrt();
                let efficiency = (direct_distance / path_length.max(0.1)) * 100.0;
                
                log::debug!("[2D A*] Path quality: 2D Length={:.1}m, Direct={:.1}m, Efficiency={:.1}%", 
                           path_length, direct_distance, efficiency);
            }
            
            Some(world_path)
        }
        None => {
            log::warn!("[2D A*] FAILED to find 2D path from {:?} to {:?}", start_grid, goal_grid);
            None
        }
    }
}
    ...
    F: FnMut(Vec3A) -> bool + Clone,
{
    ...
    log::debug!("[HIERARCHICAL] Coarse grid: {:.1}m, Fine grid: {:.1}m", coarse_grid, fine_grid);
    ...
    log::debug!("[HIERARCHICAL] Coarse path found with {} waypoints", coarse_path.len());
    ...
    for i in 0..coarse_path.len() - 1 {
        ...
        if let Some(segment_path) = advanced_astar_pathfind(
            segment_start, 
            segment_end, 
            fine_grid, 
            is_walkable.clone()
...
pub fn multi_target_pathfind_with_center_bias<F>(
    ...
    grid_size: f32,
    is_walkable: F,
) -> Option<(Vec<Vec3A>, usize)> // (경로, 선택된 목표 인덱스)
where
    F: FnMut(Vec3A) -> bool + Clone,
{
    if targets.is_empty() {
    ...
    log::debug!("[MULTI-TARGET] Searching paths to {} targets with center bias", targets.len());
    ...
    for (idx, &target) in targets.iter().enumerate() {
        ...
        if combined_score < best_score {
            ...
            if let Some(path) = advanced_astar_pathfind(start, target, grid_size, is_walkable.clone()) {
                ...
                log::debug!("[MULTI-TARGET] Target {} - distance: {:.1}m, center_bias: {:.1}, score: {:.1}", 
                           idx, direct_distance, center_bias_score, combined_score);
            }
        }
    }
    
    if let Some(path) = best_path {
        log::info!("[MULTI-TARGET] Best target found: index {}, score: {:.1}", 
                  best_target_idx, best_score);
    ...
    F: FnMut(Vec3A) -> bool + Clone,
{
    ...
    log::debug!("[MULTI-TARGET] Searching paths to {} targets", targets.len());
    ...
    if let Some(path) = best_path {
        log::info!("[MULTI-TARGET] Best target found: index {}, cost: {:.1}m", 
                  best_target_idx, shortest_cost);
        Some((path, best_target_idx))
    } else {
        None
    }
