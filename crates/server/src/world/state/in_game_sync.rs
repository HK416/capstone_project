...
/// 모든 플레이어의 로딩이 완료될 때 까지 대기하는 상태입니다.
...
impl GameWorldInGameSyncState {
    ...
    /// 플레이어의 `ActionState`를 초기화합니다.
    fn setup_player(&mut self, world: &GameWorld) {
        ...
        let spawn_positions = self.spawn_positions.as_mut().unwrap();
        ...
        for item in world.sessions.iter() {
            if let Some(data) = play_data.get(item.value()) {
                if !data.loaded {
                    ...
                    unloaded_sessions.push(item.key().clone());
                }
            }
        }

        // 모든 플레이어가 준비된 경우 또는 남은 게임 지속시간이 0인 경우
        // 다음 게임 월드 상태로 전환합니다.
        if all_player_loaded || self.remaining_time_sec <= 0.0 {
            self.is_running = false;

            // 아직 로드를 완료하지 못한 플레이어를 서버와 연결 해제합니다.
            for session in unloaded_sessions {
                // 이 함수에서 world.exit()를 호출할 경우 데드락이 발생합니다.
                // 따라서 session.close()를 호출하여 다른 스레드에서 차후 월드에서 제거되어야합니다.
                session.close();
            }
...
//--------------------------------------------------------------------------------------------
impl GameWorldInGameSyncState {
    fn broadcast_init_stage(&self, world: &GameWorld) {
...
impl GameWorldState for GameWorldInGameSyncState {
    fn on_enter(&mut self, world: &Arc<GameWorld>) {
        ...
        self.broadcast_init_stage(world);
