    ...
    match action_state {
        ActionState::Idle => match movement_state {
            ...
            MovementState::Jumping => {
                update_movement_state_timer_when_jumping(data, elapsed_time_ms)
            }
            MovementState::Landing => {
                update_movement_state_timer_when_landing(data, elapsed_time_ms)
            }
        },
        ActionState::Aiming
        | ActionState::AimAt
...
/// [`MovementState::Jumping`]일 때 [`MovementStateTimer`]를 갱신합니다.
pub fn update_movement_state_timer_when_jumping(data: &mut Player, elapsed_time_ms: u16) {
    data.movement_state_timer.0 = data.movement_state_timer.0.saturating_add(elapsed_time_ms);
...
/// [`MovementState::Landing`]일 때 [`MovementStateTimer`]를 갱신합니다.
pub fn update_movement_state_timer_when_landing(data: &mut Player, elapsed_time_ms: u16) {
    data.movement_state_timer.0 =
