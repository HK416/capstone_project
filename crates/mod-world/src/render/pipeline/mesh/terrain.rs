...
impl MeshRenderer for TerrainRenderer {
    #[inline]
    #[must_use]
    ...
    fn bind<'a>(&'a self, camera: &CameraResource, rpass: &mut wgpu::RenderPass<'a>) {
        ...
        rpass.set_vertex_buffer(2, self.mesh().attribute(&Attribute::Texcoords1).unwrap().slice(..));
    }
    ...
    PIPELINE.get_or_init(|| {
        ...
        device.create_render_pipeline(
            &wgpu::RenderPipelineDescriptor {
                label: Some("RenderPipeline(TerrainRenderer)"), 
                layout: Some(&layout), 
