...
use mod_world::render::{config_swapchain, create_surface, RenderError, DEPTH_STENCIL_FORMAT};
use winit::{error::OsError, event_loop::ActiveEventLoop, window::{Window, WindowAttributes}};
...
impl AppWindow {
    ...
    pub fn on_resized(&self, instance: &wgpu::Instance, device: &wgpu::Device) {
        ...
        *depth_buffer_view = Arc::new(device.create_texture(
            &wgpu::TextureDescriptor {
                label: Some("Depth-Buffer"), 
                dimension: wgpu::TextureDimension::D2, 
                format: DEPTH_STENCIL_FORMAT, 
