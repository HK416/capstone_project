...
use mod_parallelism::collections::Stack;
...
fn check_invalidation(historys: Vec<Vec<History>>, queue: Arc<Stack<Box<u32>>>) {
...
fn validation_main(num_threads: usize, queue: Arc<Stack<Box<u32>>>) -> Vec<History> {
...
fn thread_main(num_threads: usize, queue: Arc<Stack<u32>>) {
    let num_tests = NUM_TESTS / num_threads;
    for _ in 0..num_tests {
        if rand::random() {
            let mut val = rand::random();
