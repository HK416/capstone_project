//! 클라이언트와 서버에서 사용할 수 있는 타이머 코드를 작성합니다.
//! 
...
pub struct GameTimer {
    ...
    frame_samples: [f32; Self::NUM_SAMPLES],
...
impl GameTimer {
    /// `Stopwatch`가 저장하는 샘플 경과 시간의 갯수입니다.
    ...
    pub fn tick<'a>(&'a mut self) {
        ...
        if (self.elapsed_time_sec - elapsed_time_sec).abs() < 1.0 {
            self.frame_samples.copy_within(0..Self::NUM_SAMPLES - 1, 1);
            self.frame_samples[0] = elapsed_time_sec;
            self.cnt_frame_samples = (self.cnt_frame_samples + 1).min(Self::NUM_SAMPLES);
        }
...
impl Default for GameTimer {
    ...
    fn default() -> Self {
        ...
        Self { 
            ...
            frame_samples: [0.0; Self::NUM_SAMPLES], 
            cnt_frame_samples: 0, 
            frame_rate: 0,
