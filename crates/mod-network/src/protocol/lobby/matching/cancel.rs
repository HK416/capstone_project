//! 클라이언트가 로비 장면에 있을 때 랜덤매치 참여를 취소하기 위한 패킷을 관리합니다.
//!
...
/// 클라이언트가 서버로 보내는 랜덤매치 참가 취소 패킷입니다.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MatchCancelPacket {
    /// 사용자 식별자
    pub uid: UserId,
    /// 로그인 토큰
...
impl MatchCancelPacket {
    pub fn new(uid: UserId, token: LoginToken) -> Self {
        Self { uid, token }
    }
}

impl Packet for MatchCancelPacket {
...
mod tests {
    ...
    #[test]
    fn test_match_cancel_packet() {
        let origin = MatchCancelPacket::new(UserId::new(131543561), LoginToken::new(25161643514));
        let raw = origin.as_raw();
        let other = MatchCancelPacket::from_raw(raw);
