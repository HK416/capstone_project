...
impl Player {
    ...
    pub fn from_bytes(data: &[u8]) -> Player {
        ...
        let hp = u32::from_be_bytes(data[4..8].try_into().unwrap());
        ...
        let anim_index = u32::from_be_bytes(data[36..40].try_into().unwrap());
        let anim_timer = f32::from_be_bytes(data[40..44].try_into().unwrap());
