...
fn get_eye_mouth_color(texcoord: vec2<f32>) -> vec4<f32> {
    ...
    let row = u_material.sprite_index / 8;
    let col = u_material.sprite_index % 8;
...
fn fs_main(input: VertexOutput) -> RenderTarget {
    ...
    let tex_color = textureSample(t_main_color, s_main_color, input.uv).rgb;    
    let eye_mouth = get_eye_mouth_color(input.uv);
    if eye_mouth.a < 0.5 {
        discard;
    }

    let main_color = tex_color * eye_mouth.rgb;
